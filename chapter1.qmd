...
3. **Law of large numbers**: As your sample size grows, random fluctuations become less influential and your sample statistics get closer to the true population values.
...
1. **Random error (sampling error)**: The natural variation that occurs by chance when we select a random sample. This happens simply because we're looking at a portion rather than the whole population.
   ...
   **Standard error** measures this sampling error. It shows how much our sample statistics (like means or proportions) would vary if we repeatedly took different samples from the same population.
...
2. **Systematic error (non-sampling error)**: Occurs when our data collection methods systematically skew results in one direction. Unlike sampling error, systematic error CANNOT be reduced by increasing sample size.
   ...
   * **Response bias**: When respondents provide inaccurate answers (e.g., over-reporting income)
...
When estimating regression model parameters:
...
- These parameters show relationships between variables
...
- Key factors affecting required sample size:
  ...
  * Strength of the effects being studied (weaker effects require larger samples)
...
- **Superpopulation approach**:
  - **Population/Sample**: 50 existing US states
  - **Superpopulation**: Theoretical set of all possible territorial units of the "state" type under different historical, political, and social conditions
...
3. **Statistical Inference**: The normal distribution forms the foundation for many statistical tests and confidence intervals.## Population Parameters
...
Understanding the symbols used in statistics is essential for following statistical concepts and formulas. Here's a reference table of the most commonly used notation:

| Symbol | Description | Example |
|--------|-------------|---------|
| $n$ | Sample size (number of observations in a sample) | "We collected data from $n = 30$ participants" |
| $N$ | Population size (total number of units in the population) | "The town has $N = 5,000$ residents" |
| $\bar{x}$ | Sample mean (average of observations in a sample) | "$\bar{x} = 25.3$ years" |
| $\mu$ | Population mean (average value in the entire population) | "The population mean height $\mu = 175$ cm" |
| $s$ | Sample standard deviation | "The sample standard deviation $s = 4.2$ points" |
| $s^2$ | Sample variance | "The sample variance $s^2 = 17.64$ square points" |
| $\sigma$ | Population standard deviation | "The population standard deviation $\sigma = 15$ minutes" |
| $\sigma^2$ | Population variance | "The population variance $\sigma^2 = 225$ square minutes" |
| $\hat{p}$ | Sample proportion | "$\hat{p} = 0.35$ of respondents agreed" |
| $p$ | Population proportion | "The true proportion $p$ of left-handed people" |
| $H_0$ | Null hypothesis | "$H_0: \mu = 100$" |
| $H_1$ or $H_a$ | Alternative hypothesis | "$H_1: \mu \neq 100$" |
| $\alpha$ | Significance level (probability of Type I error) | "We used $\alpha = 0.05$ for our test" |
| $\beta$ | Probability of Type II error | "The test had $\beta = 0.2$" |
| $1-\beta$ | Statistical power | "The test had power $1-\beta = 0.8$" |
| $\sum$ | Summation | "$\sum_{i=1}^{n} x_i$ means add all values from $x_1$ to $x_n$" |
| $\rho$ | Population correlation coefficient | "The population correlation $\rho = 0.75$" |
| $r$ | Sample correlation coefficient | "The sample correlation $r = 0.72$" |
| $df$ | Degrees of freedom | "The $t$-test had $df = 29$" |
| $SE$ | Standard error | "$SE(\bar{x}) = \frac{s}{\sqrt{n}}$" |
| $CI$ | Confidence interval | "95% $CI = [42.1, 45.7]$" |
| $\hat{\theta}$ | Generic notation for an estimator of parameter $\theta$ | "$\hat{\theta}$ is our estimator" |

When reading statistical formulas and concepts, always pay attention to the distinction between:
- **Sample statistics** (typically Latin letters: $\bar{x}$, $s$, $r$)
- **Population parameters** (typically Greek letters: $\mu$, $\sigma$, $\rho$)

This convention helps clarify whether we're discussing a value calculated from our sample data or a true (usually unknown) characteristic of the entire population.
