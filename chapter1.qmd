...
A good estimator **$\hat{\theta}$** should have the following properties:  

#### 1️⃣ Unbiasedness: No Systematic Error  
A good estimator should, on average, be correct. This means that if we take many different samples and compute their estimates, the average of these estimates should be equal to the true population value **$\theta$**.  
...
#### 2️⃣ Efficiency: The Least Variability  
An efficient estimator has **the smallest possible variance**, meaning it gives results that are more tightly clustered around the true value **$\theta$**.  

#### 3️⃣ Consistency: Gets Closer to the True Value with More Data  
As the sample size increases, a **consistent estimator** gets closer to **$\theta$**.  

#### 4️⃣ Sufficiency: Uses All the Information in the Data  
A **sufficient estimator** extracts all the relevant information from the sample.  

---

### Why Do Estimates Vary? (Distribution of Sample Statistics)  

Since different samples give different estimates, we study the **distribution of an estimator** (called a **sampling distribution**) to understand how much it varies.  

#### Key Ideas About Sample Distributions  
1. **Sample estimates vary**—Different samples give different estimates.  
2. **The variation follows a pattern**—The **sampling distribution** describes how estimates behave.  
3. **Larger samples reduce variation**—Bigger samples give more stable estimates.  
4. **Sampling distributions allow us to quantify uncertainty**—This is why confidence intervals and hypothesis tests work.  

#### Example: Sample Mean  
If we take many samples and compute their means, they will form a **sampling distribution** centered around **$\theta = \mu$**.  
...
Statistical errors can compromise the validity and reliability of research findings. Understanding these errors is crucial for designing robust studies, analyzing data accurately, and drawing appropriate conclusions.
...
- **Definition**: Natural variability that occurs when taking random samples
- **Example**: Average height measurements varying slightly across different random samples of 30 university students
- **Properties**:
  - Unavoidable but manageable
  - Decrease as sample size increases
  - Can be quantified using margin of error
  - Follow predictable patterns based on sampling theory

#### Non-Random (Systematic) Sampling Errors (Bias)
- **Selection bias**: Sample doesn't represent the target population
  - **Example**: Phone survey reaching only people with landlines, missing those who use only cell phones
  
- **Undercoverage**: Systematic exclusion of specific groups
  - **Example**: Campus survey during daytime classes missing evening students
  
- **Self-selection bias**: Skewed results from participant choice
  - **Example**: Customer satisfaction surveys completed primarily by those with strong opinions
  
- **Non-response bias**: Specific groups less likely to respond
  - **Example**: Busy individuals not completing questionnaires, leading to overrepresentation of those with more free time

### Measurement Errors

#### Random Measurement Errors
- **Definition**: Unpredictable fluctuations in measurements due to random variability (often called "noise")
- **Explanation**: Similar to the error term (ε) in regression models, these are the unexplained variations that occur by chance
- **Examples**: 
  - Slightly different blood pressure readings due to a shaky hand
  - Small fluctuations in instrument readings due to environmental factors
  - Minor variations when the same person takes repeated measurements
- **Key characteristic**: Tend to cancel out with repeated measurements (positive errors balance negative errors)

#### Systematic Measurement Errors
- **Calibration errors**: Consistent inaccuracies in measurement tools
  - **Example**: Scale consistently adding 2 pounds to every measurement
  
- **Observer bias**: Researcher expectations influencing data collection
  - **Example**: Unconsciously recording data in ways that support a hypothesis
  
- **Social desirability bias**: Participants giving socially acceptable responses
  - **Example**: Underreporting junk food consumption in nutrition studies

### Inference Errors

#### Hypothesis Testing Errors
- **Type I error (False Positive)**: Concluding an effect exists when it doesn't
  - **Example**: Claiming a new drug works when its benefits are due to chance
  - **Context**: Related to significance level (α)
  
- **Type II error (False Negative)**: Missing an effect that actually exists
  - **Example**: Concluding a new drug doesn't work when it actually helps patients
  - **Context**: Related to statistical power (1-β)
  
- **Type III error**: Solving the wrong problem altogether
  - **Definition**: Asking the wrong question or focusing on an irrelevant hypothesis
  - **Example**: Studying whether a marketing strategy increases website traffic when the real issue is whether it increases sales
...
- **Underpowered studies**: Insufficient sample size to detect true effects
  ...
  - **Prevention**: Conducting proper power analysis before data collection

### Model Specification Errors

#### Variable Selection Errors
- **Omitted variable bias**: Excluding important influential variables
  - **Example**: Studying exercise and weight loss without accounting for diet
  
- **Confounding**: External factors influencing both predictor and outcome
  - **Example**: People who exercise also eating healthier, affecting weight loss results
  - **Solution**: Randomization, stratification, or statistical control techniques

#### Assumption Errors
- **Definition**: Using statistical tests that don't align with data characteristics
- **Example**: Assuming normal distribution when data is heavily skewed
- **Prevention**: Verifying assumptions through diagnostic tests before analysis

### Data Processing and Analysis Errors

#### Data Handling Errors
- **Data entry errors**: Mistakes during recording
  - **Example**: Typing "1050" instead of "150" for weight
  - **Prevention**: Double entry, range checks, validation procedures
  
- **Missing data problems**: Improper handling of incomplete data
  - **Example**: Calculating average test scores while ignoring absent students
  - **Solutions**: Imputation methods, sensitivity analyses

#### Analysis Execution Errors
- **Software bugs or user errors**: Mistakes in using statistical software
  - **Example**: Selecting the wrong test in analysis software
  - **Prevention**: Code review, reproducible workflows, documentation
...
- **Correlation-causation fallacy**: Mistaking correlation for causation
  - **Example**: Concluding chocolate consumption causes Nobel Prizes because they correlate
  - **Prevention**: Understanding research design limitations, proper framing of results

#### Reporting and Communication Errors
- **p-hacking**: Running multiple analyses until finding significance
  - **Example**: Analyzing data in 20 different ways and reporting only significant outcomes
  - **Prevention**: Pre-registration, correction for multiple comparisons
  
- **Cherry-picking**: Selectively reporting supportive results
  - **Example**: Diet pill company publishing only successful studies
  - **Prevention**: Comprehensive reporting, registered reports

## Strategies to Minimize Statistical Errors

1. **Proper research design**: Careful planning before data collection
   - Use appropriate sampling methods
   - Randomize when possible
   - Control for confounding variables

2. **Adequate sample size**: Base on power calculations
   - Consider practical significance, not just statistical significance
   - Account for potential dropouts or missing data

3. **Pre-registration**: Declare analysis plans before examining data
   - Specify primary outcomes and methods
   - Document any deviations from the plan

4. **Transparency**: Share data and methods openly
   - Provide access to raw data when possible
   - Clearly document analytical decisions

5. **Replication**: Encourage independent verification
   - Conduct internal replications
   - Support external replication efforts
   - Value reproducibility as much as novelty

6. **Continuous education**: Stay updated on best practices
   - Follow field-specific guidelines
   - Engage with methodological innovations

Understanding and addressing these errors ensures research findings are trustworthy, preventing poor decisions across fields like medicine, public policy, education, and business.
:::
