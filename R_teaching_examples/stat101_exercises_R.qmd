...
DM <- c(3, 4, 5, 6, 7, 8, 10)
LHI <- c(15.5, 14.25, 13.5, 13.5, 13, 12.75, 8.5)

DM <- c(3, 4, 5, 6, 7, 9, 10)
LHI <- c(15.5, 14.25, 13.5, 13.5, 13, 12.25, 10.75)

DM <- c(4, 4, 5, 6, 7, 9)
LHI <- c(15.5, 14.25, 13.5, 13.5, 13.25, 11.25)
...
### Example 5: Anxiety vs. Test Score
...
performance <- rev(c(72, 68, 74, 77, 82, 78, 85, 80))      # Exam scores (%)
anxiety
performance
...
#| echo: true
#| warning: false
# Visualize errors for an arbitrary line
library(ggplot2)

# Create a simple model
data <- data.frame(anxiety = anxiety, performance = performance)
simple_model <- lm(performance ~ anxiety, data = data)
summary(simple_model)
data$predicted <- predict(simple_model)

# Plot with error lines
ggplot(data, aes(x = anxiety, y = performance)) +
  ...
  labs(
    title = "Visualizing Errors in Regression",
    subtitle = "Dashed lines show errors (residuals) from each point to the regression line",
...
```{r}
#| echo: true
# Adjusted data for more realistic correlation
anxiety <- c(2.5, 3.2, 4.1, 4.8, 5.6, 6.3, 7.0, 7.9)
performance <- c(82, 84, 78, 75, 72, 71, 67, 65)

# Recalculate with adjusted data
mean_y_new <- mean(performance)  # 74.25
var_y_new <- var(performance)     # 49.07
sd_y_new <- sd(performance)       # 7.005

# Calculate new covariance
deviations_x <- anxiety - mean(anxiety)
deviations_y <- performance - mean(performance)
cov_xy_new <- sum(deviations_x * deviations_y) / 7  # -10.05

# New correlation
cor_new <- cor(anxiety, performance)
cat("Adjusted correlation:", cor_new, "\n")
