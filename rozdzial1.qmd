...
Wybrane schematy losowego doboru próby:
...
## Podstawowe Pojęcia Estymacji Statystycznej  

### Parametry populacji vs. Statystyki próby  
...
#### Przykłady parametrów populacyjnych i statystyk próby  

| **Koncepcja**         | **Parametr populacji (Nieznany, stały)** | **Statystyka próby (Obliczona z próby)** |
|----------------------|---------------------------------|---------------------------------|
| Średnia (średnia arytmetyczna) | **$\theta = \mu$** (prawdziwa średnia populacji) | **$\hat{\theta} = \bar{x}$** (średnia z próby) |
| Wariancja           | **$\theta = \sigma^2$** (prawdziwa wariancja populacji) | **$\hat{\theta} = s^2$** (wariancja z próby) |
| Proporcja           | **$\theta = p$** (prawdziwa proporcja) | **$\hat{\theta} = \hat{p}$** (proporcja w próbie) |
...
#### Przykład: Oszacowanie średniego wzrostu dorosłych  
...
### Jakie cechy powinien mieć dobry estymator?  

Dobry **$\hat{\theta}$** powinien spełniać następujące warunki:  

#### 1️⃣ Nieobciążoność (Unbiasedness): Brak systematycznego błędu  
Dobry estymator powinien, **średnio**, dawać poprawny wynik. Oznacza to, że jeśli wielokrotnie pobieramy różne próby i obliczamy ich estymaty, to ich średnia powinna być równa prawdziwej wartości **$\theta$**.  

Estymator **$\hat{\theta}$** jest **nieobciążony**, jeśli:  
$$
E(\hat{\theta}) = \theta
$$  

#### 2️⃣ Efektywność (Efficiency): Najmniejsza wariancja  
Efektywny estymator ma **najmniejszą możliwą wariancję**, czyli jego wyniki są bardziej skupione wokół prawdziwej wartości **$\theta$**.  

#### 3️⃣ Zbieżność (Consistency): Coraz lepsze oszacowanie przy większej próbie  
Gdy liczba obserwacji rośnie, **zbieżny estymator** coraz lepiej przybliża **$\theta$**.  

#### 4️⃣ Dostateczność (Sufficiency): Wykorzystuje wszystkie informacje  
**Estymator dostateczny** uwzględnia całą dostępną informację zawartą w próbie.  

---

### Dlaczego estymaty się różnią? (Rozkład statystyk próby)  

Ponieważ różne próby dają różne estymaty, badamy **rozkład estymatora** (zwany **rozkładem próbkowania**), aby zrozumieć, jak bardzo wyniki mogą się różnić.  

#### Kluczowe pojęcia dotyczące rozkładu statystyk  
1. **Estymaty zmieniają się między próbami**—Różne próby dają różne wyniki.  
2. **Ta zmienność podlega pewnym regułom**—**Rozkład próbkowania** opisuje, jak estymaty się zachowują.  
3. **Większa próba oznacza mniejszą zmienność**—Duże próby dają stabilniejsze wyniki.  
4. **Rozkład próbkowania pozwala oszacować niepewność**—To podstawa przedziałów ufności i testów statystycznych.  

#### Przykład: Średnia z próby  
Jeśli wielokrotnie pobieramy próby i obliczamy ich średnie, ich wartości tworzą **rozkład próbkowania** skupiony wokół **$\theta = \mu$**.  

Matematycznie, średnia z próby to:  
$$
\hat{\theta} = \bar{x} = \frac{1}{n} \sum_{i=1}^{n} x_i
$$  
gdzie **$x_1, x_2, ..., x_n$** to wartości w próbie.  
...
> Uwaga
>
> Rozkład normalny, często nazywany „krzywą dzwonową”, to jedno z najważniejszych pojęć w statystyce. Opisuje, jak dane są rozłożone, i pojawia się wszędzie w rzeczywistym świecie—np. w przypadku wzrostu ludzi, wyników testów, a nawet błędów pomiarowych. Krzywa ma kształt dzwonu, z większością danych skupionych wokół średniej i coraz mniejszą liczbą danych w miarę oddalania się od środka.

### Co Definiuje Krzywą Dzwonową?
Krzywa dzwonowa jest określona przez dwie kluczowe liczby:

1. **Średnia ($\mu$)**: To wartość średnia, która wyznacza środek krzywej.
2. **Odchylenie standardowe ($\sigma$)**: Określa, jak bardzo dane są rozproszone. Małe odchylenie standardowe oznacza, że dane są blisko średniej, a duże—że są bardziej rozrzucone.

### Dlaczego Jest Ważna?

Rozkład normalny jest niezwykle użyteczny, ponieważ:

1. **Zasada 68-95-99.7**:
   - Około 68% danych znajduje się w odległości 1 odchylenia standardowego od średniej.
   - Około 95% danych znajduje się w odległości 2 odchyleń standardowych.
   - Około 99,7% danych znajduje się w odległości 3 odchyleń standardowych.

2. **Centralne Twierdzenie Graniczne**:  
   Wyobraź sobie, że masz jakieś dane, które nie wyglądają jak typowa krzywa dzwonowa (np. mogą być rozrzucone nieregularnie). Jeśli zaczniesz wielokrotnie losować próbki z tych danych i za każdym razem obliczać ich średnią, to te średnie zaczną układać się w kształt krzywej dzwonowej—nawet jeśli oryginalne dane nie miały takiego kształtu!  

Innymi słowy:
   
   - Niezależnie od tego, jak wyglądają Twoje dane na początku,  
   - Jeśli będziesz brał średnie z wielu małych grup tych danych,  
   - To te średnie będą tworzyć krzywą dzwonową, gdy zbierzesz ich wystarczająco dużo.
   
To właśnie dlatego rozkład normalny jest tak ważny—nawet jeśli coś nie wydaje się "normalne" na pierwszy rzut oka, to średnie z tego czegoś często zachowują się zgodnie z krzywą dzwonową.

3. **Narzędzia Statystyczne**: Wiele testów i metod statystycznych opiera się na rozkładzie normalnym do przewidywania i wyciągania wniosków.

### Zobaczmy to w Działaniu!
Oto jak można stworzyć i zwizualizować krzywą dzwonową za pomocą R:

```{R}
# Załaduj niezbędną bibliotekę
library(ggplot2)

# Zdefiniuj średnią i odchylenie standardowe
mean <- 0
sd <- 1

# Stwórz sekwencję wartości x
x <- seq(-4, 4, length.out = 1000)

# Oblicz odpowiadające wartości y (gęstość prawdopodobieństwa)
y <- dnorm(x, mean, sd)

# Narysuj krzywą normalną
ggplot(data.frame(x, y), aes(x, y)) +
  geom_line(color = "blue", linewidth = 1) +
  labs(title = "Rozkład Normalny (Krzywa Dzwonowa)",
       x = "Wartości",
       y = "Gęstość Prawdopodobieństwa") +
  theme_minimal()
```

Ten kod generuje gładką krzywą dzwonową wyśrodkowaną na 0 z odchyleniem standardowym równym 1. Możesz zmienić wartości `mean` i `sd`, aby zobaczyć, jak zmienia się krzywa!
:::
...
Testowanie hipotez to sposób na sprawdzenie, czy dane twierdzenie dotyczące grupy (lub populacji) jest prawdopodobnie prawdziwe, czy nie. To trochę jak bycie detektywem: zbierasz dowody (dane) i decydujesz, czy dowody potwierdzają twoje twierdzenie.

---

### Przykład: Testowanie, czy moneta jest sprawiedliwa

Załóżmy, że chcesz sprawdzić, czy moneta jest sprawiedliwa (czyli czy ma równe szanse na wypadnięcie orła lub reszki).

#### Krok 1: Zadaj pytanie badawcze
- **Pytanie**: Czy moneta jest sprawiedliwa? (Czy prawdopodobieństwo orła = 0,5?)

#### Krok 2: Ustal dwie hipotezy
- **Hipoteza zerowa (H₀)**: Moneta jest sprawiedliwa. (Prawdopodobieństwo orła = 0,5)
- **Hipoteza alternatywna (H₁)**: Moneta nie jest sprawiedliwa. (Prawdopodobieństwo orła ≠ 0,5)

Hipotezę zerową traktujemy jako domyślne założenie, czyli „niewinny, dopóki nie udowodniono winy”. Hipoteza alternatywna to to, co próbujemy udowodnić.

#### Krok 3: Zbierz dane
- Rzucasz monetą 100 razy i otrzymujesz **65 orłów**.

#### Krok 4: Przeanalizuj dane
- Gdyby moneta była sprawiedliwa, oczekiwałbyś około 50 orłów w 100 rzutach.
- Liczba orłów powinna podlegać **rozkładowi dwumianowemu** (to wzór, który opisuje, jak wyniki powinny wyglądać przy wielokrotnym rzucaniu monetą).
- Odchylenie standardowe (miara tego, jak bardzo wyniki mogą się różnić) wynosi 5.
- Uzyskanie 65 orłów oznacza wynik **3 odchylenia standardowe** od oczekiwanego wyniku 50 orłów. To bardzo nietypowe!

#### Krok 5: Oblicz wartość p
- **Wartość p** to prawdopodobieństwo uzyskania 65 lub więcej orłów (lub wyniku równie ekstremalnego), jeśli moneta byłaby sprawiedliwa.
- W tym przypadku wartość p jest bardzo mała (mniej niż 0,01), co oznacza, że taki wynik jest bardzo mało prawdopodobny, gdyby moneta była sprawiedliwa.

#### Krok 6: Podjąć decyzję
- Jeśli wartość p jest mała (zwykle mniejsza niż 0,05), **odrzucamy hipotezę zerową**.
- Tutaj wartość p jest bardzo mała, więc wnioskujemy, że moneta **nie jest sprawiedliwa**.

---

### Ogólne Kroki Testowania Hipotez

1. **Sformułuj hipotezy**:
   - Hipoteza zerowa (H₀): Domyślne twierdzenie (brak efektu).
   - Hipoteza alternatywna (H₁): Twierdzenie, które testujesz.

2. **Wybierz poziom istotności (α)**:
   - To próg, który decyduje, czy wartość p jest wystarczająco mała, aby odrzucić H₀. Typowe wartości to 0,05 lub 0,01.

3. **Zbierz dane**:
   - Zbierz dowody (np. rzuć monetą 100 razy).

4. **Oblicz statystykę testową i wartość p**:
   - **Statystyka testowa** mierzy, jak bardzo twoje dane odbiegają od tego, co przewiduje H₀.
   - **Wartość p** mówi, jak prawdopodobne jest uzyskanie twoich danych (lub czegoś bardziej ekstremalnego), jeśli H₀ jest prawdziwa.

5. **Podejmij decyzję**:
   - Jeśli wartość p < α, odrzuć H₀ i przyjmij H₁.
   - Jeśli wartość p ≥ α, nie odrzucaj H₀ (ale to nie oznacza, że H₀ jest prawdziwa).

---

### Intuicja Za Testowaniem Hipotez

Testowanie hipotez można porównać do procesu sądowego:
- **Hipoteza zerowa (H₀)**: Oskarżony jest niewinny.
- **Hipoteza alternatywna (H₁)**: Oskarżony jest winny.
- **Dane**: Dowody przedstawione w sądzie.
- **Wartość p**: Jak silne są dowody przeciwko oskarżonemu.
- **Poziom istotności (α)**: Standard dowodu potrzebny do skazania.

Jeśli dowody są wystarczająco silne (wartość p < α), odrzucamy H₀ (skazujemy oskarżonego). Jeśli nie, nie odrzucamy H₀ (ale to nie oznacza, że udowodniliśmy jego niewinność).

---

### Częste Błędy w Interpretacji Wartości p

1. **Wartość p NIE jest prawdopodobieństwem, że H₀ jest prawdziwa**.
   - To prawdopodobieństwo uzyskania twoich danych (lub czegoś bardziej ekstremalnego), **jeśli H₀ jest prawdziwa**.

2. **Wartość p NIE jest prawdopodobieństwem popełnienia błędu**.
   - Nie mówi, jakie jest prawdopodobieństwo, że się mylisz, odrzucając H₀.

3. **Nieodrzucenie H₀ NIE oznacza, że jest ona prawdziwa**.
   - To tylko oznacza, że nie ma wystarczających dowodów, aby ją odrzucić.

4. **Mała wartość p NIE oznacza, że wynik jest ważny w praktyce**.
   - To tylko oznacza, że wynik jest statystycznie istotny. Efekt może być w rzeczywistości bardzo mały.

5. **Wartość p zależy od wielkości próby**.
   - Przy bardzo dużych próbach nawet małe różnice mogą być statystycznie istotne.

---

### Rodzaje Błędów w Testowaniu Hipotez

1. **Błąd I rodzaju (Fałszywie pozytywny)**:
   - Odrzucenie H₀, gdy jest ona prawdziwa.
   - Przykład: Skazanie niewinnej osoby.
   - Prawdopodobieństwo tego błędu = α (poziom istotności).

2. **Błąd II rodzaju (Fałszywie negatywny)**:
   - Nieodrzucenie H₀, gdy jest ona fałszywa.
   - Przykład: Uniewinnienie winnej osoby.
   - Prawdopodobieństwo tego błędu = β.

3. **Moc testu (1 - β)**:
   - Prawdopodobieństwo poprawnego odrzucenia H₀, gdy jest ona fałszywa.
   - Moc testu rośnie wraz z większą próbą i większym efektem.

---

### Kluczowe Wnioski

- Testowanie hipotez pomaga zdecydować, czy dane potwierdzają twierdzenie.
- Wartość p mówi, jak bardzo zaskakujące są twoje dane, zakładając, że H₀ jest prawdziwa.
- Zawsze interpretuj wartość p ostrożnie i unikaj częstych błędów.
- Pamiętaj: Istotność statystyczna ≠ ważność praktyczna.
...
W tym modelu: - $\beta_0$ (wyraz wolny) i $\beta_1$ (współczynnik nachylenia) to parametry, które szacujemy. - $\epsilon$ reprezentuje składnik losowy, który przechwytuje zmienność niewyjaśnioną przez model. - Zakładamy, że składnik losowy ma rozkład normalny o średniej 0 i wariancji $\sigma^2$.
...
## Appendix G: Treści uzupełniające (*)
...
Używając próby do poznania populacji, nieuchronnie napotykamy błąd statystyczny:

> **Błąd statystyczny** to różnica między oszacowaniem z próby a prawdziwą wartością w populacji.

Błędy statystyczne mogą zagrozić trafności i wiarygodności wyników badań. Zrozumienie tych błędów jest kluczowe dla projektowania solidnych badań, właściwej analizy danych i wyciągania odpowiednich wniosków.

## Rodzaje błędów statystycznych

### Błędy próbkowania

#### Losowe błędy próbkowania
- **Definicja**: Naturalna zmienność występująca przy pobieraniu losowych prób
- **Przykład**: Średnie pomiary wzrostu różniące się nieznacznie w różnych losowych próbach 30 studentów uniwersytetu
- **Właściwości**:
  - Nieuniknione, ale możliwe do zarządzania
  - Zmniejszają się wraz ze wzrostem wielkości próby
  - Można je oszacować za pomocą marginesu błędu
  - Podlegają przewidywalnym wzorcom opartym na teorii próbkowania

#### Nielosowe (systematyczne) błędy próbkowania (Obciążenie)
- **Błąd selekcji**: Próba nie reprezentuje populacji docelowej
  - **Przykład**: Ankieta telefoniczna docierająca tylko do osób posiadających telefony stacjonarne, pomijając tych, którzy korzystają wyłącznie z telefonów komórkowych
  
- **Niedostateczne pokrycie**: Systematyczne wykluczanie określonych grup
  - **Przykład**: Ankieta na kampusie przeprowadzana tylko podczas dziennych zajęć, pomijająca studentów wieczorowych
  
- **Błąd autoselekcji**: Zniekształcone wyniki wynikające z wyboru uczestników
  - **Przykład**: Ankiety zadowolenia klientów wypełniane głównie przez osoby o silnych opiniach
  
- **Błąd braku odpowiedzi**: Określone grupy mniej skłonne do odpowiedzi
  - **Przykład**: Zajęte osoby nie wypełniają kwestionariuszy, co prowadzi do nadreprezentacji osób z większą ilością wolnego czasu

### Błędy pomiaru

#### Losowe błędy pomiaru
- **Definicja**: Nieprzewidywalne wahania w pomiarach spowodowane losową zmiennością (często nazywaną "szumem")
- **Wyjaśnienie**: Podobne do składnika błędu (ε) w modelach regresji, są to niewyjaśnione wahania występujące przypadkowo
- **Przykłady**: 
  - Nieznacznie różne odczyty ciśnienia krwi z powodu drżącej ręki
  - Małe wahania odczytów instrumentów z powodu czynników środowiskowych
  - Drobne różnice, gdy ta sama osoba wykonuje powtarzane pomiary
- **Kluczowa cecha**: Mają tendencję do znoszenia się przy powtarzanych pomiarach (błędy dodatnie równoważą błędy ujemne)

#### Systematyczne błędy pomiaru
- **Błędy kalibracji**: Konsekwentne niedokładności narzędzi pomiarowych
  - **Przykład**: Waga konsekwentnie dodająca 2 funty do każdego pomiaru
  
- **Błąd obserwatora**: Oczekiwania badacza wpływające na zbieranie danych
  - **Przykład**: Nieświadome rejestrowanie danych w sposób potwierdzający hipotezę
  
- **Błąd pożądanej społecznie odpowiedzi**: Uczestnicy udzielający społecznie akceptowanych odpowiedzi
  - **Przykład**: Zaniżanie zgłaszanego spożycia śmieciowego jedzenia w badaniach żywieniowych

### Błędy wnioskowania

#### Błędy testowania hipotez
- **Błąd typu I (Fałszywie pozytywny)**: Wnioskowanie o istnieniu efektu, gdy go nie ma
  - **Przykład**: Twierdzenie, że nowy lek działa, gdy jego korzyści wynikają z przypadku
  - **Kontekst**: Związany z poziomem istotności (α)
  
- **Błąd typu II (Fałszywie negatywny)**: Przeoczenie efektu, który faktycznie istnieje
  - **Przykład**: Wnioskowanie, że nowy lek nie działa, gdy faktycznie pomaga pacjentom
  - **Kontekst**: Związany z mocą statystyczną (1-β)
  
- **Błąd typu III**: Rozwiązywanie niewłaściwego problemu
  - **Definicja**: Zadawanie niewłaściwego pytania lub skupianie się na nieistotnej hipotezie
  - **Przykład**: Badanie, czy strategia marketingowa zwiększa ruch na stronie internetowej, gdy prawdziwym problemem jest to, czy zwiększa sprzedaż

#### Problemy z mocą statystyczną
- **Badania z niewystarczającą mocą**: Niewystarczająca wielkość próby do wykrycia prawdziwych efektów
  - **Przykład**: Testowanie programu odchudzania z zaledwie 5 uczestnikami
  - **Zapobieganie**: Przeprowadzanie odpowiedniej analizy mocy przed zbieraniem danych

### Błędy specyfikacji modelu

#### Błędy wyboru zmiennych
- **Błąd pominiętej zmiennej**: Wykluczenie ważnych wpływowych zmiennych
  - **Przykład**: Badanie ćwiczeń i utraty wagi bez uwzględnienia diety
  
- **Czynniki zakłócające**: Czynniki zewnętrzne wpływające zarówno na predyktor, jak i na wynik
  - **Przykład**: Osoby, które ćwiczą, również jedzą zdrowiej, co wpływa na wyniki utraty wagi
  - **Rozwiązanie**: Randomizacja, stratyfikacja lub techniki kontroli statystycznej

#### Błędy założeń
- **Definicja**: Stosowanie testów statystycznych, które nie są zgodne z charakterystyką danych
- **Przykład**: Zakładanie rozkładu normalnego, gdy dane są silnie skośne
- **Zapobieganie**: Weryfikacja założeń poprzez testy diagnostyczne przed analizą

### Błędy przetwarzania i analizy danych

#### Błędy obsługi danych
- **Błędy wprowadzania danych**: Pomyłki podczas rejestrowania
  - **Przykład**: Wpisanie "1050" zamiast "150" dla wagi
  - **Zapobieganie**: Podwójne wprowadzanie, kontrole zakresu, procedury walidacji
  
- **Problemy z brakującymi danymi**: Niewłaściwe postępowanie z niekompletnymi danymi
  - **Przykład**: Obliczanie średnich wyników testów przy ignorowaniu nieobecnych uczniów
  - **Rozwiązania**: Metody imputacji, analizy wrażliwości

#### Błędy wykonania analizy
- **Błędy oprogramowania lub użytkownika**: Pomyłki w korzystaniu z oprogramowania statystycznego
  - **Przykład**: Wybór niewłaściwego testu w oprogramowaniu analitycznym
  - **Zapobieganie**: Przegląd kodu, odtwarzalne przepływy pracy, dokumentacja

### Błędy interpretacji i raportowania

#### Błędy wnioskowania przyczynowego
- **Błąd korelacji-przyczynowości**: Mylenie korelacji z przyczynowością
  - **Przykład**: Wnioskowanie, że konsumpcja czekolady powoduje Nagrody Nobla, ponieważ są skorelowane
  - **Zapobieganie**: Zrozumienie ograniczeń projektu badawczego, właściwe formułowanie wyników

#### Błędy raportowania i komunikacji
- **p-hacking**: Przeprowadzanie wielu analiz, aż do znalezienia istotności
  - **Przykład**: Analizowanie danych na 20 różnych sposobów i raportowanie tylko istotnych wyników
  - **Zapobieganie**: Wstępna rejestracja, korekta dla wielokrotnych porównań
  
- **Wybieranie wisienek** (cherry-picking): Selektywne raportowanie wyników potwierdzających
  - **Przykład**: Firma produkująca tabletki odchudzające publikująca tylko udane badania
  - **Zapobieganie**: Kompleksowe raportowanie, zarejestrowane raporty

## Strategie minimalizacji błędów statystycznych

1. **Właściwy projekt badawczy**: Staranne planowanie przed zbieraniem danych
   - Stosowanie odpowiednich metod próbkowania
   - Randomizacja gdy to możliwe
   - Kontrola zmiennych zakłócających

2. **Odpowiednia wielkość próby**: Oparta na obliczeniach mocy
   - Rozważenie praktycznej istotności, nie tylko istotności statystycznej
   - Uwzględnienie potencjalnych rezygnacji lub brakujących danych

3. **Wstępna rejestracja**: Deklaracja planów analizy przed badaniem danych
   - Określenie głównych wyników i metod
   - Dokumentowanie wszelkich odstępstw od planu

4. **Przejrzystość**: Otwarte udostępnianie danych i metod
   - Zapewnienie dostępu do surowych danych, gdy to możliwe
   - Jasne dokumentowanie decyzji analitycznych

5. **Replikacja**: Zachęcanie do niezależnej weryfikacji
   - Przeprowadzanie wewnętrznych replikacji
   - Wspieranie zewnętrznych wysiłków replikacyjnych
   - Wartościowanie odtwarzalności tak samo jak nowości

6. **Ciągła edukacja**: Aktualizacja najlepszych praktyk
   - Przestrzeganie wytycznych specyficznych dla dziedziny
   - Zaangażowanie w innowacje metodologiczne

Zrozumienie i rozwiązywanie tych błędów zapewnia wiarygodność wyników badań, zapobiegając podejmowaniu złych decyzji w takich dziedzinach jak medycyna, polityka publiczna, edukacja i biznes.
:::
